...
class AbstractFileDownloader(object):
    ...
    def __init__(self, args):
        ...
        self.chromosomes = [str(j + 1) for j in range(22)] + ['X', 'Y']
        self.chromosomes = self.chromosomes[:min(self.numChromosomes,24)]
        ...
        self.accessions = {
            '1': 'CM000663.1', '2': 'CM000664.1', '3': 'CM000665.1',
            '4': 'CM000666.1', '5': 'CM000667.1', '6': 'CM000668.1',
            '7': 'CM000669.1', '8': 'CM000670.1', '9': 'CM000671.1',
            '10': 'CM000672.1', '11': 'CM000673.1', '12': 'CM000674.1',
            '13': 'CM000675.1', '14': 'CM000676.1', '15': 'CM000677.1',
            '16': 'CM000678.1', '17': 'CM000679.1', '18': 'CM000680.1',
            '19': 'CM000681.1', '20': 'CM000682.1', '21': 'CM000683.1',
            '22': 'CM000684.1', 'X': 'CM000685.1', 'Y': 'CM000686.1',
        }
        self.studyMap = {}
        with open('integrated_call_samples.20130502.ALL.ped', 'r') as csvfile:
            filereader = csv.DictReader(csvfile, delimiter=b"\t")
            for row in filereader:
                self.studyMap[row['Individual ID']] = row['Population']

        self.bamMap = {}
        self.baiMap = {}
        individual_re = re.compile('data/([A-Z0-9]+)/.*')
        with open('alignment.index', 'r') as csvfile:
            filereader = csv.DictReader(csvfile, delimiter=b"\t")
            for row in filereader:
                bam = row['BAM FILE']
                if "unmapped" in bam or "chrom" in bam:
                    continue
                bai = row['BAI FILE']
                parsed = individual_re.search(bam)
                individual = parsed.group(1)
                self.bamMap[individual] = bam
                self.baiMap[individual] = bai
    ...
    def getBamBaseUrl(self):
        return os.path.join(self.getBaseUrl(), 'phase3/')
    ...
    def _downloadVcf(self, chromosome):
        ...
        suffix="shapeit2_mvncall_integrated"
        if chromosome == "Y":
            suffix="integrated"

        sourceFileName = (
            "ALL.chr{}.phase3_{}_{}"
            ".20130502.genotypes.vcf.gz").format(chromosome, suffix, version_str)
        ...
        with open(localTempFileName, 'wb') as outfile:
            while True:
    ...
    def _downloadBamBySample(self, sample):
        destFileName = os.path.join(
            self.dirName, "{}.bam".format(sample))
        baseUrl = self.getBamBaseUrl()
        sampleUrl = os.path.join(self.getBamBaseUrl(),self.bamMap[sample])
        sourceFileName = os.path.basename(sampleUrl)
    ...
    def downloadBams(self):
        for sample in self.samples:
            self._downloadBamBySample(sample)
...
class NcbiFileDownloader(AbstractFileDownloader):
    ...
    """
    def getBaseUrl(self):
        return 'ftp://ftp-trace.ncbi.nih.gov/1000genomes/ftp'
...
class EbiFileDownloader(AbstractFileDownloader):
    ...
    """
    def getBaseUrl(self):
        return 'ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp'


class S3FileDownloader(AbstractFileDownloader):
    """
    Downloads files from EBI
    """
    def getBaseUrl(self):
        return 'http://1000genomes.s3.amazonaws.com'
